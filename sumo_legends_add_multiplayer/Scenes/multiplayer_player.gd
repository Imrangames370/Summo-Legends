...
#newly made multiplayer player script
#gotta change it to not update um the skins or anything for hotseat or i could just not initialzie it 
#for hotseat. really so long as i keep scenes seperate and dont insiniate it as a child should be good,
# Add at the top
var is_network_master := false
var received_positions: Array = []
var received_rotations: Array = []
var network_sync_rate: float = 0.1
...
func create_player_ball_scheduled_for_creation_count():
	#only triggered when the player gets a plus or multiplcation modifier or on game start if 
	VariableDock.player_alive[player_number] = 2 #mark that player is now alive again
	VariableDock.player_current_lives[player_number] += VariableDock.player_ball_scheduled_for_creation_count[player_number] #increase player lives equal to amount of newly created balls
	VariableDock.player_number_of_alive_balls[player_number] += VariableDock.player_ball_scheduled_for_creation_count[player_number]
	for i in range(VariableDock.player_ball_scheduled_for_creation_count[player_number]):
		var pos = update_sphere_positions(i)
		create_sphere(pos)
		print("creating additional objects")
		VariableDock.player_ball_scheduled_for_creation_count[player_number] = 0 #set to 0 as now creation has been triggered. must be set to 0 otherwise it will loop and keep creating.
...
func delete_player_ball_scheduled_for_deletion_count():
	...
	round(number_to_delete)
	if number_to_delete > VariableDock.player_number_of_alive_balls[player_number]:
		return
	...
	for i in range(actual_delete - 1, -1, -1):
		var idx = indices[i]
		var obj = objects[idx]
	...
	if VariableDock.player_current_lives[player_number] < 1: #player is dead
...
func _ready():
	if multiplayer.has_multiplayer_peer():
		is_network_master = str(name) == str(multiplayer.get_unique_id())
...
func _physics_process(delta):
	...
	if is_network_master:
		...
		if _network_timer >= network_sync_rate:
			var positions = []
			var rotations = []
			for obj in objects:
				var body_rid = obj[0]
				...
				rotations.append(trans.basis.get_rotation_quaternion())
			_update_server_state.rpc(positions, rotations)
			_network_timer = 0.0
		...
		for i in range(objects.size()):
			if i < received_positions.size() and i < received_rotations.size():
				var body_rid = objects[i][0]
				PhysicsServer3D.body_set_state(
					body_rid, 
					PhysicsServer3D.BODY_STATE_TRANSFORM,
					Transform3D(Basis(received_rotations[i]), received_positions[i])
				)
...
func _handle_movement_input_left_right(direction):
					for obj in objects:
						var body_rid = obj[0]
...
func _handle_movement_input_forward_backward(direction):
					for obj in objects:
						var body_rid = obj[0]
...
func _handle_movement_input_jump():
					for obj in objects:
						var body_rid = obj[0]
						# Get the current linear velocity
						available_jumps -= 1
...
func _process_local_input():
	action_prefix = "player_%d_" % player_number
	if Input.is_action_pressed(action_prefix + "move_right"):
		_handle_movement_input_left_right(1)
	if Input.is_action_pressed(action_prefix + "move_left"):
		_handle_movement_input_left_right(-1)
	if Input.is_action_pressed(action_prefix + "move_forward"):
		_handle_movement_input_forward_backward(1)
	if Input.is_action_pressed(action_prefix + "move_backward"):
		_handle_movement_input_forward_backward(-1)
	if Input.is_action_pressed(action_prefix + "jump"):
		_handle_movement_input_jump()
	for obj in objects:
		var body_rid = obj[0]
		...
		check_rapid_input(action_prefix + "move_forward", Vector3(1 * VariableDock.player_dash_force_multiplier + VariableDock.player_radius[player_number], 0, 0))
		check_rapid_input(action_prefix + "move_backward", Vector3(-1 * VariableDock.player_dash_force_multiplier + VariableDock.player_radius[player_number], 0, 0))
		check_rapid_input(action_prefix + "move_right", Vector3(0, 0, 1 * VariableDock.player_dash_force_multiplier + VariableDock.player_radius[player_number]))
		check_rapid_input(action_prefix + "move_left", Vector3(0, 0, -1 * VariableDock.player_dash_force_multiplier + VariableDock.player_radius[player_number]))
		check_rapid_input(action_prefix + "jump", Vector3(0, 2 * VariableDock.base_empowered_jump_modifier_multiplier_all_players + VariableDock.player_radius[player_number], 0))
...
func _update_server_state(positions: Array, rotations: Array):
	if not is_network_master:
		received_positions = positions
		received_rotations = rotations

func action_rapid_input_set():
	actions = [
						action_prefix + "move_forward",
						action_prefix + "move_backward",
						action_prefix + "move_right",
						action_prefix + "move_left",
						action_prefix + "jump"
				]
...
func _rally_ability():
	...
	for i in range(1, objects.size()):
		...
		current_energy -= 25
	print("Rally ability activated for player %d")

func _input(event):
	if event.is_action_pressed(action_prefix + "rally") and VariableDock.game_in_play == 2 and player_number != -1 and objects.size() > 1:
		_rally_ability()
	if event.is_action_pressed(action_prefix + "stealth") and VariableDock.stealth_enabled == 1 and player_number != -1 and VariableDock.game_in_play == 2 and current_energy >= 100:
	...
	if event.is_action_pressed("multiplayer_connect"):
			if player_number == -1:
	...
	if player_number != -1 and VariableDock.game_in_play == 2:
		if event.is_action_pressed(action_prefix + "cycle_trigger_assist_shift"):
			shift_pressed = true
		elif event.is_action_released(action_prefix + "cycle_trigger_assist_shift"):
			shift_pressed = false
		if event.is_action_pressed(action_prefix + "cycle_mode"):
				# Reset the timer
			if menu_timer.is_stopped():
				...
				menu_timer.stop()
				menu_timer.start()
		if not shift_pressed and event.is_action_pressed(action_prefix + "cycle_mode"):  # Pressing E key cycles modes
		...
		elif event.is_action_pressed(action_prefix + "cycle_mode") and shift_pressed and menu_instance:
			_activate_selected()
